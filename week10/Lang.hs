...
stdLib = Map.fromList
  [("tail", Fun $ \ v -> case v of Ls (_:ls) -> Ok $ Ls ls
   ...
   ("len", Fun $ \v -> 
   	case v of
   		Ls (x) -> Ok $ I $ getLength x
   		_      -> Error "you can't get the length of an empty list")]
...
evalFunc :: Ast -> EnvUnsafe Env (Val -> Unsafe Val)
evalFunc x = do 
	res <- eval x
	case res of
		Fun x -> return x
...
eval (Div x y) = do
	...
	int2 <- evalInt y
	case int2 of
		0 -> err "you can't divide by zero"
		_ -> do 
			return (I (div int1 int2))
...
eval (Var name) = getVar name --week 8 lang 4
...
local :: (e -> e) -> EnvUnsafe e a -> EnvUnsafe e a
local changeEnv comp = EnvUnsafe (\env -> runEnvUnsafe comp (changeEnv env))
