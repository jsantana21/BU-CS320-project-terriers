...
decimalParser :: Parser Float
decimalParser = do x <- some digit
                   literal "."
                   y <- some digit
                   return $ read (x ++ "." ++ y)

floatParser :: Parser Float
floatParser = do r <- (literal "-") <||> decimalParser
                 case r of
                  Left _ -> fmap (0-) decimalParser
                  Right n -> return n

-- parse brackets
brackets first b last = do
    first
    xs <- b
    last
    return xs
...
-- parse comments, throw them away
comment :: Parser ()
comment = do
    literal "--"
    rep $ sat (/= '\n')
